...
// DetectInstanceAndProject detects both GitLab instance URL and project path from .git/config.
// Returns (instanceURL, projectPath, error).
//
// If instance URL cannot be determined, both return values will be empty strings.
// If project path cannot be determined but instance can, instance URL will be populated
// and project path will be empty.
func DetectInstanceAndProject(dir string) (string, string, error) {
	...
	// Extract instance URL and project path
	instanceURL, err := extractInstanceURL(remoteURL)
	if err != nil {
		return "", "", err
	}
	...
	if err != nil {
		// Return instance even if project extraction failed
		return instanceURL, "", nil
	}

	return instanceURL, projectPath, nil
}
...
// extractInstanceURL extracts the base instance URL from various Git remote URL formats.
// Returns normalized URL (e.g., "https://gitlab.com", "https://git.a1tis.ru").
//
// Supported formats:
//   - https://gitlab.com/group/project.git → https://gitlab.com
//   - git@gitlab.com:group/project.git → https://gitlab.com
//   - ssh://git@git.a1tis.ru/group/project → https://git.a1tis.ru
//   - http://gitlab.example.com:8080/group/project → http://gitlab.example.com:8080
//   - git@gitlab.com:group/project → https://gitlab.com
//
// Returns error if the URL format cannot be parsed.
func extractInstanceURL(remoteURL string) (string, error) {
	if remoteURL == "" {
		return "", nil
	}
	...
	if strings.HasPrefix(remoteURL, "git@") || strings.HasPrefix(remoteURL, "ssh://git@") {
		...
		// Handle port: git@host:port:path
	...
	if host == "" {
		return "", fmt.Errorf("cannot extract host from URL: %s", remoteURL)
	}
	...
	return NormalizeInstanceURL(instanceURL), nil
}
...
// DetectInstanceForFile detects the GitLab instance URL for a specific file.
...
func DetectInstanceForFile(filePath string) (string, string, error) {
	...
	if err == nil {
		...
		if info.IsDir() {
			...
			return DetectInstanceAndProject(dir)
		}

		// .git is a file - this is a git submodule or worktree
		// Try to detect from the submodule's own config
		instance, project, err := DetectInstanceAndProject(dir)
		if err == nil && instance != "" {
